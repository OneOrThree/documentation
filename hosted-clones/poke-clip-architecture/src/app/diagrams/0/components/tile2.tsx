...
export default function Tile2({ d, cids, styles }: { d: Tile2Data; cids: string[]; styles: Tile2Styles }) {
  return (
    <div data-cid={cids[0]} className="h-[5.2rem] min-h-11.5 border border-solid border-color-001 flex relative max-h-37.5 pt-1.5 pb-[0.3125rem] px-[0.5625rem] rounded-md flex-col justify-center gap-px text-center bg-surface cursor-pointer" id={d.id}>
      <span data-cid={cids[1]} className="block text-[0.8125rem] font-bold leading-[0.9375rem]">
        {d.text}
      </span>
      <span data-cid={cids[2]} className={cn("block text-color-003 text-[0.625rem] leading-[0.75rem]", styles.className)}>
        {d.text2}
      </span>
