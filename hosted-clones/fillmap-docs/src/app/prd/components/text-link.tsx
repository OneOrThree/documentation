...
export default function TextLink({ d, cids, styles }: { d: TextLinkData; cids: string[]; styles: TextLinkStyles }) {
  return (
    <a data-cid={cids[0]} className={cn("border-b border-solid border-b-surface-2 grid py-[1.0625rem] px-2 items-center gap-4 grid-rows-[31.5px] cursor-pointer grid-cols-[46px_1fr_auto] max-md:grid-cols-[34px_269px]", styles.className)} data-component="link" href="/prd/msg-178-prd">
      {" "}
      ...
      <span data-cid={cids[4]} className="flex min-w-0 justify-end items-center gap-3 text-muted text-[0.8125rem] leading-[1.4375rem] whitespace-nowrap text-nowrap max-md:col-start-2">
        {" "}
        <span data-cid={cids[5]} className={cn("border block max-w-33 py-[0.1875rem] px-[0.5625rem] rounded-[999px] overflow-hidden [font-family:ui-monospace,_SFMono-Regular,_'SF_Mono',_'JetBrains_Mono',_Menlo,_monospace] text-[0.6875rem] leading-[1.1875rem] tracking-[0.88px] uppercase", styles.className3)}>
          {d.label3}
        </span>
        {" →\n"}
      </span>
