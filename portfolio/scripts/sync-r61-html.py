...
가공: 헤더·푸터·외부 문서 링크·로컬 경로 제거, IA는 버튼·범례만 한 줄로, 저니는 v4 흐름도 형식을 유지한다.
...
assert "게스트 포함 모든 사용자가 동일하게 이용" in j, "사용자 여정 게스트 전체 기능 정책이 빠짐"
...
print(f"synced: diagrams/01-ia.html, 02-journey.html(v4 형식 유지) + 원본 drawio/svg + 01-ia.drawio.xml({n} nodes, {e} edges) — 이어서 npm run build 또는 node scripts/build-diagrams.mjs")
