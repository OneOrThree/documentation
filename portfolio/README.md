...
HTML 형식에는 SVG·XML 연결 그래프를 요구하지 않으며, 기존 그래프의 `문서 보기` 토글을 표시하지 않습니다. 유저 저니의 상세 여정 12개는 같은 HTML 안에 있습니다. IA는 `주요 기능`과 `세부 기능` 두 버튼으로 펼침 상태를 바꾸며, 개별 가지 펼침·구조 선택·검색은 제공하지 않습니다.

유저 저니에서 IA를 참조하는 링크는 같은 공개 폴더의 `01-ia.html#focus=…`를 사용합니다. 별도 명세·draw.io 파일은 함께 배포하지 않습니다. `public/diagrams/`는 빌드 때 다시 생성되므로 직접 편집하지 않습니다.
